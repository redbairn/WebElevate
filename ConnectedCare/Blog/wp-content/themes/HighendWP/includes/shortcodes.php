...
if ( !function_exists('hb_flip_boxes_shortcode') ) {
	function hb_flip_boxes_shortcode($params = array()) {
		...
		$flip .= !empty( $button_url ) ? '<a href="' . $button_url . '" class="hb-button no-three-d' . $button_color . '" target="_blank">' . $button_text . '</a>' : "";
	...
	function hb_owl_blog_carousel_shortcode($params = array()) {
		...
		if ( $blog_posts->have_posts() ) :
			...
			while ( $blog_posts->have_posts() ) : $blog_posts->the_post();
				...
				$output .= '<a href="#" class="hb-post-title">' . get_the_title() . '</a>';
